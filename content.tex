...
Zusätzlich gibt es die Solo-Variante \gdq{Nullensolo} und den
Spieltyp \gdq{Armut}. Das Nullensolo kann als Lust- oder Pflichtsolo
...
\subsection*{2.3.5 (Ergänzung)}

Beim Farbsolo gibt es keine Schweinchen.

\subsection*{2.3.8 (neu)}

Bei der Solo-Variante \gdq{Nullensolo} gilt die Rangfolge wie beim
Fleischlosen (Assesolo), siehe dazu Punkt 2.3.7.

\subsection*{2.3.9 (neu)}

Beim Spieltyp \gdq{Armut} gilt die Rangfolge wie beim Normalspiel,
siehe dazu 2.3.4.
...
9.2). Ein Fehler beim Geben der Karten wird mit einem Idiotenstern geahndet
(siehe 7.4).
...
ein Spieler vorgeführt wird) können keine Bockspiele sein. Ein Bockspiel, das
...
\begin{itemize}
    ...
    \item{eine Spielaktion ausführt obwohl er nicht an der Reihe ist. Dies gilt
        ...
        Vorbehaltes.}
    \item{nicht regelkonform austeilt (z.B. Karten nicht in Dreierpaketen
        ausgeteilt, Abheben versäumt).}
    \item{beim Austeilen eine Karte aufwirft. Das Aufwerfen einer Karte beim
        Mischen wird nicht geahndet. Unmittelbar nach dem Abheben müssen alle
        Karten verdeckt sein und müssen verdeckt bleiben.}
